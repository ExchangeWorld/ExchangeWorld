                ...
                $.ajax({
                    ...
                    success: function (response) {
                        for(var i=0; i<response.length;i++){
                            $('#comment_area').append('<div><img class="owner" data-value="'+response[i]["commenter"]+'" src="' + response[i]["commenterPhoto"] + '" style="width: 30px; height: 30px;"> '+ response[i]["comment"] +'</div>');
                        }
                    },
                    error: function (xhr, ajaxOption, thrownError) {
                        alert(thrownError);
                        ...
                        success: function (response) {
                            ...
                            for (var i = 0; i < response.length; i++) {
                                if (response[i]["status"] == 0) { // Exchanging
                                    $('#Exchanging').append('<div class="col-md-3 searchResult" style="padding: 0px; padding-top: 0px; padding-bottom: 0px; border: 0px; background: #fff; margin: 0px;" data-value="' + response[i]["gid"] + '"><img src="' + response[i]["photoPath"] + '"style="width: 70px; height: 70px;" class="img-thumbnail" alt="..."></div>');
                                }
                                /*  else{
                                        If "status" != 0
...
$(document).ready(function () {
    ...
    $("#leftSideSwitch").on("click", ".submit", function (event) {
        ...
        if(marker!=null && post_categories !=-1){
            $.ajax({
                type: "GET",
